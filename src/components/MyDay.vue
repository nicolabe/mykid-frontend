...
export default {
  ...
  computed: {
    sleepMessage: function() {
      const message = this.myDay.daily_messages.filter(
        message => message.msg_type === "sleep"
      )[0];
      if (message) {
    ...
    dailyMessage: function() {
      const messages = this.myDay.daily_messages.filter(
        message => message.msg_type === "daily"
      );
      return messages
        .map(message =>
